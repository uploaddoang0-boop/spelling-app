...
document.getElementById('btn-reset-mastery').addEventListener('click', () => { 
    if(confirm("Ulangi progres penguasaan dari 0%?")) {
...
function applyCustomTarget() {
    ...
    if(val > 0) {
        targetLimit = val; targetBtns.forEach(b => b.classList.remove('active'));
        btnCustomTarget.classList.add('active'); btnCustomTarget.textContent = val; 
        initSession();
    } else { inputCustomTarget.value = ''; }
}
...
function updateMasteryUI() {
    ...
    if(masteredCount === currentRawData.length) { sHtml += ` &nbsp;<span style="color:var(--correct); font-weight:bold;">[ TUNTAS ]</span>`; }
...
function initSession() {
    ...
    currentRawData = rawData; currentCategoryTitle = title;
    updateMasteryUI(); 
    ...
    rawData.forEach(item => {
        const rec = wordDatabase[item.word.toLowerCase()];
        if (!rec || rec.correct <= rec.wrong) { learningPool.push(item); }
    });

    if(learningPool.length === 0) { learningPool = [...rawData]; }
    let limit = targetLimit === "ALL" ? learningPool.length : Math.min(targetLimit, learningPool.length);

    let sessionData = [];
    if (activeCategory === "WEAK") {
        sessionData = shuffleArray(learningPool.slice(0, limit)); 
    } else {
        sessionData = shuffleArray([...learningPool]).slice(0, limit); 
    }
...
function prepareSession(dataArray) {
    ...
    sessionQueue = sessionWords; sessionTotal = sessionQueue.length; sessionCompleted = 0; sessionAttempts = 0; sessionCorrectAttempts = 0;
    
    idleText.classList.add('hidden'); wordInput.classList.remove('hidden'); sessionStats.classList.remove('hidden');
    btnListen.classList.remove('hidden'); btnSubmit.classList.remove('hidden');
...
function loadNextWord() {
    if (sessionQueue.length > 0) {
        ...
        wordInput.value = ''; feedbackArea.innerHTML = ''; wordInput.classList.remove('shake-animation');
